...
@section('content')
    ...
    <div class="container">
        <section class="content">
            <form method="POST" action="{{ route('store-post') }}" aria-label="Create Post" enctype="multipart/form-data">
            @csrf
                <div>
                    <div>
                        <h2 class="box-title">Створити оголошення</h2>
                    ...
                    <div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="post_type" class="form-select">
                                <option value="1" @if (old('post_type') === 1) {{ 'selected' }} @endif>Пропозиція</option>
                                <option value="0" @if (old('post_type') === 0) {{ 'selected' }} @endif>Потреба</option>
                            </select>
                        </div>
                        <div class="form-group form-group-tag">
                            <label>Тип допомоги</label>
                            <input name="tagify" id="tagify">
                        </div>
                        <div class="form-group">
                            <label>Опис</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="checkbox" name="is_map" checked>
                        </div>
                        ...
                        <div class="form-group">
                            <input id="map_long" type="hidden" class="form-control" name="map_long" placeholder="" value="{{old('map_long')}}">
                        </div>
                        <div class="form-group">
                            <input id="map_lat" type="hidden" class="form-control" name="map_lat" placeholder="" value="{{old('map_lat')}}">
                        </div>
                        <div class="form-group">
                            <label>Контакти</label>
                            <input type="contact_info" class="form-control" name="contact_info" placeholder="" value="{{old('contact_info')}}">
                        </div>
                        <div class="form-group">
                            <label for="image_link">Фото</label>
                            <input type="file" name="image_link" class="form-control-file">
                        </div>
                    </div>
                    <div>
                        <button class="btn btn-success pull-right">Створити оголошення</button>
                    </div>
                </div>
            </form>
        </section>
